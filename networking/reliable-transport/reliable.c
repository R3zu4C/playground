...
void rel_recvpkt(rel_t *r, packet_t *pkt, size_t n)
{
    ...
    if (n == 8) print_pkt(pkt, "receive ack", n);
    if (n == 12) print_pkt(pkt, "receive EOF", n);
    if (n > 12) print_pkt(pkt, "receive data", n);
    ...
    if (EOF_RECV(r->flags) && pkt_seqno >= r->eof_seqno) return;
...
void rel_read(rel_t *r)
{
    ...
    {
        if (r->send_buffer[first_free % r->window_size].allocated) first_free++;
        else break;
    }
    ...
    if (LAST_ALLOCATED_ALREADY_SENT(r->flags)) newest_seqno = first_free;
    else newest_seqno = first_free - 1;
    ...
    fprintf(stderr, "%5d, %s = %d\n", getpid(), "READ Len", recieved_bytes);
...
void rel_output(rel_t *r)
{
    ...
    while (r->recv_buffer[r->recv_seqno % r->window_size].allocated)
    {
        ...
        size_t written = conn_output(r->c, &(s->segment) + r->already_written, s->len - r->already_written);
    ...
    if (EOF_RECV(r->flags))
    {
        ...
        if (buffer_empty) SET_ALL_WRITTEN(r->flags);
    }
}
...
void rel_timer()
{
    ...
    if (EOF_READ(rel_list->flags) && all_ackwoledged) SET_ALL_SENT_ACKNOWLEDGED(rel_list->flags);

    // Call rel_destroy if session ended.
    if (EOF_RECV(rel_list->flags) && EOF_READ(rel_list->flags) && ALL_SENT_ACKNOWLEDGED(rel_list->flags) && ALL_WRITTEN(rel_list->flags)) rel_destroy(rel_list);
}
