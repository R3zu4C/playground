    ...
    for (i = 1; i < ncevents; i++)
    {
        if (cevents[i].revents & (POLLIN | POLLERR | POLLHUP))
        {
            if ((c = evreaders[i]) && !c->delete_me)
            {
                ...
                else if (cevents[i].fd == c->nfd && (cevents[i].revents & (POLLERR | POLLHUP)))
                {
                    char addr[NI_MAXHOST] = "unknown";
                    char port[NI_MAXSERV] = "unknown";
...
void do_client(struct config_client *cc)
{
    conn_mkevents();
...
void do_server(struct config_server *cs)
{
    serverconf = cs;
    conn_mkevents();
...
int main(int argc, char **argv)
{
    ...
    if (optind + 2 != argc || c.window < 1 || c.timeout < 10 || (opt_server && opt_client) || (!(opt_server || opt_client) && opt_unix))
        usage();
    c.timer = c.timeout / 5;
    local = argv[optind];
    ...
    if (opt_server)
    {
        ...
        cs.c = c;
        if (get_address(&cs.dest, 0, 0, opt_unix ? AF_UNIX : AF_INET, remote) < 0 || get_address(&ss, 1, 1, AF_INET, local) < 0 || (cs.udp_socket = listen_on(1, &ss)) < 0)
            exit(1);
        do_server(&cs);
    }
    else if (opt_client)
    {
        ...
        struct sockaddr_storage ss;
        cc.c = c;
        if (get_address(&cc.server, 0, 1, AF_INET, remote) < 0 || get_address(&ss, 1, 0, opt_unix ? AF_UNIX : AF_INET, local) < 0 || (cc.listen_socket = listen_on(0, &ss)) < 0)
            exit(1);
        do_client(&cc);
    }
    else
    {
        ...
        cn->rfd = 0;
        cn->wfd = 1;
        if (get_address(&sr, 0, 1, AF_INET, remote) < 0 || get_address(&sl, 1, 1, sr.ss_family, local) < 0 || (cn->nfd = listen_on(1, &sl)) < 0)
            exit(1);
